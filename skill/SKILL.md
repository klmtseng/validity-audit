...
1. **冷審(先做):** 只給碼 + headline 主張,**不給提示清單、不說作者發現了什麼**。要它自己推威脅模型、找問題。
   ——這才量得到「真獨立」(被指路的 reviewer 只會找你指的地方)。理想上**換不同模型家族(或人)**做冷審,
   因為同模型 reviewer 與建造者共用盲點。
...
5. reviewer 報告存 `audit/independent_review.md`,內部審計存 `results/leak_audit.md`;**記錄每次 audit 的 hit/miss/誤報**(別只靠一次成功)。
